rule fastqc:
    ...
    conda:
        "../../../../envs/qc.yaml"
    log:
        "{pipeline}/logs/fastqc/{state}/{sample}.log"
    shell:
        """
        echo "[$(date)] Starting fastqc for {wildcards.sample} ({wildcards.state})" > {log}
        fastqc --threads {threads} \
                --outdir {wildcards.pipeline}/qc/fastqc/{wildcards.state} \
                {input.r1} {input.r2} \
        >> {log} 2>&1
        echo "[$(date)] Finished fastqc for {wildcards.sample} ({wildcards.state})" >> {log}
        """
